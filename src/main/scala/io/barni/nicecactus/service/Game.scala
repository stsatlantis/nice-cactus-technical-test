...
import io.barni.nicecactus.model.Player.{ `3CPO`, Bender, Human }
...
class Game(
  ...
  private def selectMode(): IO[ScoreBoard] =
    for {
  ...
  private def gameLoop(scoreBoard: ScoreBoard): IO[Unit] = {

    def loop(round: Int, scoreBoard: ScoreBoard): IO[Unit] =
      for {
        ...
        playerScore <- roundService.winner(scoreBoard, p1Move, p2Move)
  ...
  private def printWinner(player: Option[Player]): IO[Unit] = {
    player match {
  ...
  private def printScoreBoard(scoreBoard: ScoreBoard): IO[Unit] =
    for {
      _ <- console.putStrLn(s"""${boardPlayer1Lens.get(scoreBoard)} - ${boardPlayer1ScoreLens.get(scoreBoard)}
  ...
  private def checkNewRound(): IO[Boolean] =
    for {
  ...
  private def printWelcome(): IO[Unit] =
    for {
