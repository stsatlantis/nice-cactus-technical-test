...
object Player {
  ...
  implicit def humanCanMove(implicit console: Console): CanMove[Human] = new CanMove[Human] {
    override def move(input: Human): IO[Move] = for {
  ...
  implicit def robotCanMove(implicit moves: Moves, random: Random, console: Output): CanMove[Robot] = new CanMove[Robot] {
    override def move(input: Robot): IO[Move] =
      for {
